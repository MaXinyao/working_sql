...
--     round_num FLOAT64 OPTIONS(description="回合数"),
--     holder_out FLOAT64 OPTIONS(description="使用holder次数"),
--     )
-- PARTITION BY
--     date
-- OPTIONS (
...
USING 
    (SELECT
        ...
        ,IFNULL(SUM(rev.round_num),0) AS round_num
        ,IFNULL(SUM(rev.holder_out),0) AS holder_out
    FROM (
        SELECT 
            create_date
            ,campaign_name 
    ...
    LEFT JOIN
        (SELECT 
            ...
            ,IFNULL(SUM(round_num),0) AS round_num
            ,IFNULL(SUM(holder_out),0) AS holder_out
        FROM `blockpuzzle-f21e1.learnings_data_warehouse_android.analytics_dm_action_userPrimaryMetric_di_*` 
        WHERE date = DATE_ADD(@run_date, INTERVAL -2 DAY)
        GROUP BY 1,2,3) rev
    ON u.date = rev.date
...
WHEN MATCHED THEN
    UPDATE SET rev = n.rev
        ...
        ,holder_out = n.holder_out
WHEN NOT MATCHED THEN
    INSERT (create_date, campaign_name, campaign_id, date, first_country, media_source, ua_cost, living_days, retended,
        rev, duration_sec, crush_rows, crush_times, best_record, combo_times, game_num, round_num, holder_out) 
